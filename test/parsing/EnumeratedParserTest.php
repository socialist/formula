...
class EnumeratedParserTest extends TestCase {
  ...
  public function testInvalidStart(): void {
    ...
    $this->expectExceptionMessage((new ParsingException(ParsingException::PARSING_ERROR_GENERIC, $firstToken))->getMessage());
  ...
  public function testInvalidLastDelimiter(): void {
    ...
    $parser = new EnumeratedParser(new TypeParser(), Token::CURLY_BRACKETS_OPEN, Token::COMMA, Token::CURLY_BRACKETS_CLOSED, false, false);
    $this->expectException(ParsingException::class);
    $this->expectExceptionMessage((new ParsingException(ParsingException::PARSING_ERROR_TOO_MANY_DELIMITERS, $firstToken->next()->next()->next()))->getMessage());
    $parser->parse($firstToken);
  }
  ...
  public function testWrongDelimiter(): void {
    ...
    $this->expectException(ParsingException::class);
    $this->expectExceptionMessage((new ParsingException(ParsingException::PARSING_ERROR_MISSING_DELIMITERS, $firstToken->next()->next()))->getMessage());
    $parser->parse($firstToken);
  }
